...
const validaId = (res, id) => {
  if (!mongoose.Types.ObjectId.isValid(id)) {
    res.status(422).send({ error: " O ID está inválido" })
    return;
  }
};
...
const jogoExiste = (res, jogo) => {
  if (!jogo) {
    res.status(404).send({ erro: " O jogo não foi encontrado!" })
    return;
  }
};
...
const validaJogo = (res, jogo) => {
  if (!jogo || !jogo.nome || !jogo.imagem) {
    res.status(400).send({ error: " O jogo está inválido!" })
    return;
  }
};
...
app.delete("/jogos/:id", async (req, res) => {
  ...
  res.send({ message: "O jogo foi excluído com sucesso!" });
});
